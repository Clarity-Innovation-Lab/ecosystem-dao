...
(define-constant err-insufficient-liquid-capacity (err u3010))
...
(define-public (vote (amount uint) (for bool) (proposal principal))
	(let
		...
		(asserts!
			(> (- (get user-balance historical-values) (get locked-balance historical-values)) u0)
			err-insufficient-liquid-capacity
		)
		(asserts!
