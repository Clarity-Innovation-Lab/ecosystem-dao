...
(define-constant err-sale-not-ended (err u3003))
...
(define-public (buy (amount uint))
	(let ((new-total (+ (var-get total-allocation) amount)))
		...
		(try! (contract-call? .ede000-governance-token edg-mint amount (as-contract tx-sender)))
...
(define-public (refund)
	(let ((amount (get-unclaimed-allocation tx-sender)))
		(asserts! (> block-height (var-get end-height)) err-sale-not-ended)
		...
		(try! (contract-call? .ede000-governance-token edg-burn amount (as-contract tx-sender)))
...
(define-public (claim)
	(let ((amount (get-unclaimed-allocation tx-sender)))
		...
		(map-delete unclaimed-allocations tx-sender)
		(contract-call? .ede000-governance-token transfer amount (as-contract tx-sender) tx-sender none)
	)
)
