...
import { EDE008FundedProposalSubmissionClient } from "./ede008-funded-proposal-submission-client.ts";
...
export class Utils {
  ...
  setup = (chain: Chain, accounts: Map<string, Account>): {
    ...
    contractEDP009: string;
    contractEDP010: string;
    contractEDP011: string;
    contractEDP012: string;
    contractEDP013: string;
    ...
    ede008FundedProposalSubmissionClient: EDE008FundedProposalSubmissionClient;
    ...
    const contractEDP009 = accounts.get("deployer")!.address + '.edp009-enable-treasury';
    const contractEDP010 = accounts.get("deployer")!.address + '.edp010-transfer-stx-assets';
    const contractEDP011 = accounts.get("deployer")!.address + '.edp010-transfer-009-assets';
    const contractEDP012 = accounts.get("deployer")!.address + '.edp010-transfer-010-assets';
    const contractEDP013 = accounts.get("deployer")!.address + '.edp010-transfer-013-assets';
    ...
    const contractEDE007 = accounts.get("deployer")!.address + '.ede007-snapshot-proposal-voting-v4';
    const contractEDE008 = accounts.get("deployer")!.address + '.ede008-funded-proposal-submission-v4';
    ...
    const ede007SnapshotProposalVotingClient = new EDE007SnapshotProposalVotingClient(chain, deployer, 'ede007-snapshot-proposal-voting-v4');
    const ede008FundedProposalSubmissionClient = new EDE008FundedProposalSubmissionClient(chain, deployer, 'ede008-funded-proposal-submission-v4');
